...
# --- 1. เปลี่ยนชื่อตัวแปรเป็น food_list เพื่อหลีกเลี่ยงการซ้ำกับระบบ ---
...
if st.session_state.mode == "start":
    ...
    if st.button("🎡 เริ่มหมุนวงล้อสุ่มหมวด"):
        # สุ่มในตัวแปรปกติก่อน ค่อยเก็บลง session_state
        temp_list = list(data_source["Street Food"])
...
# --- 4. ส่วนแบบทดสอบ ---
    ...
    if st.button("ส่งคำตอบ"):
        correct_order = [i["name"] for i in sorted(st.session_state.food_list, key=lambda x: x['cal'], reverse=True)]
